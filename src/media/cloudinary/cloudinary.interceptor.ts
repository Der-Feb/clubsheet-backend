  ...
  async intercept(context: ExecutionContext, next: CallHandler): Promise<Observable<any>> {
    ...
    if (inputPayload && inputPayload.logo) {
      ...
      if (inputPayload.logo.startsWith('data:image/')) {
        try {
          const uploadResult = await this.cloudinaryService.uploadLogoFromBase64(inputPayload.logo);
          // Overwrite the base64 string with the clean Cloudinary URL
          inputPayload.logo = uploadResult.secure_url;
        } catch (error) {
          console.log(error);
          throw new BadRequestException('Cloudinary base64 image upload failed');
        }
      }
