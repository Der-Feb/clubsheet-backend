...
export class FeatureGuard implements CanActivate {
  ...
  public async canActivate(ctx: ExecutionContext): Promise<boolean> {
    ...
    if (!clubId) {
      throw new UnauthorizedException('FeatureGuard requires ActiveMembershipGuard to run first.');
    }
    ...
    if (missing.length > 0) {
      throw new ForbiddenException(`Feature(s) not enabled for this club: ${missing.join(', ')}`);
    }
